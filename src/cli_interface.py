...
from src.indexer_utils import chunk_markdown, chunk_python, get_source_files
...
class Cli:
    ...
    def index(self, max_chunk_size: int = 2000) -> None:
        ...
        for file in tqdm(all_files, desc="Chunking"):
            if file in markdown_files:
                all_chunks.extend(
                    chunk_markdown(
                        file_path=file, max_chunk_size=max_chunk_size
                    )
                )
        ...
        print(
            f"Ingestion complete! Indexed {len(all_chunks)} "
    ...
    def _find_k_chunks(self, query: str, k: int = 5) -> List[MinimalSource]:
        ...
        best_chunks: List[MinimalSource] = bm25.get_top_n(
            tokenized_query, all_chunks, n=k)
    ...
    def search_dataset(
        ...
        for key, questions in questions_dict.items():
            ...
            for question in questions:
                ...
                result_chunks: List[MinimalSource] = []
                if query:
                    try:
    ...
    def answer_dataset(
        ...
        if not input_path.exists():
            print(
                f"Error: File '{student_search_results_path}' "
                "does not exist."
            )
            return
    ...
    def evaluate(
        ...
        def overlap_process(
            retrieved: Dict[str, Any], expected: Dict[str, Any]
        ) -> float:
            ret_path = normalize_path(retrieved.get("file_path", ""))
            exp_path = normalize_path(expected.get("file_path", ""))
            if ret_path != exp_path:
                return 0.0

            start_inter = max(
                retrieved.get("first_character_index", 0),
                expected.get("first_character_index", 0)
            )
            end_inter = min(
                retrieved.get("last_character_index", 0),
                expected.get("last_character_index", 0)
            )

            overlap_length: float = max(0, end_inter - start_inter)
        ...
        student_results_list = student_data.get("search_results", [])
        student_search_map = {
            res.get("question_id"): res for res in student_results_list
        }

        rag_questions: List[Dict[str, Any]] = gt_data.get("rag_questions", [])
        ...
        for c in cutoffs:
            ...
            for q in valid_gt_questions:
                ...
                student_res = student_search_map.get(q_id)
                retrieved_list = (
                    student_res.get("retrieved_sources", [])
                    if student_res else []
                )
            ...
            final_macro_recall = (
                ...
                if total_with_sources > 0 else 0.0
            )
